...
export function versorSetLocationAtPoint(tr: ITransform, lnglat: LngLat, point: Point, fixedBearing = false, panDelta?: Point): void {
    ...
    // {@link angularCoordinatesRadiansToVector} (x = sin(lng)·cos(lat), y = sin(lat),
    ...
    if (fixedBearing) {
        // Near the pole the swing-only longitude becomes ill-conditioned (gimbal lock) and the
        // grabbed point slips off the finger. Model the globe as having traction under the finger:
        // when the pole is centered the view is azimuthal — a "dial" — so the finger's *angle*
        // around the pole's screen position drives longitude, while its *radial* distance drives
        // latitude (handled by the swing). Blend the two by proximity of the center to the pole.
        ...
        // there rather than at 90. The ramp must be anchored on that reachable limit: anchored at
        // 90 it would saturate around 0.63, leaving the ill-conditioned swing longitude a vote
        // exactly where it is numerical noise, which randomizes the spin direction.
        ...
        // cross(r, panDelta) / |r|². Using the real pixel delta (not a round-tripped previous
        // finger position) avoids the catastrophic cancellation that randomizes the sign at the
        // exact pole. Skip it when the finger is within ~20px of the pole, where the angle is noise.
        let dLngDial = 0;
